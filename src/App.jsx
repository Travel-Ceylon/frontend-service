...
function App() {
  ...
  return (
    <>
      {user && <Navbar />}
      <Routes>
        <Route path='/' element={user ? <Navigate to={"/registration"} /> : <Login />} />

        <Route path='/registration'
          element={(user?.serviceType) ? <Navigate to={`/profile/${user.serviceType}`} /> : <Registration />} />
        ...
        <Route path='/profile/guide' element={user ? <GuideAdmin /> : <Navigate to={"/"} />} />
